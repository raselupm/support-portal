...
async function getData(id: string) {
  const product = await redis.get<DocProduct>(`doc_product:${id}`)
...
export default async function ProductArchivePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const data = await getData(id)
  if (!data) notFound()

  const { product, articles } = data
      ...
      <div className="flex-1 bg-gray-50">
        <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <p className="text-sm text-gray-500 mb-8">
            {articles.length} article{articles.length !== 1 ? 's' : ''} in this product
          </p>

          {/* Article list */}
          {articles.length === 0 ? (
            <div className="text-center py-16 text-gray-400">
              <p className="text-sm">No articles in this product yet.</p>
            </div>
          ) : (
