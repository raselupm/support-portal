...
export default function ProductsClient({ initialProducts }: { initialProducts: DocProduct[] }) {
  const [products, setProducts] = useState<DocProduct[]>(initialProducts)
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setAdding(true)
    try {
      const res = await fetch('/api/admin/docs/products', {
        method: 'POST',
  ...
  async function handleDelete(id: string) {
    if (!confirm('Delete this product? Articles using it will keep the product name but the product will no longer be selectable.')) return
    setDeleting(id)
    try {
      const res = await fetch(`/api/admin/docs/products/${id}`, { method: 'DELETE' })
  ...
  return (
    <div className="space-y-6 max-w-xl">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h2 className="text-sm font-semibold text-gray-900 mb-4">Add Product</h2>
              ...
              {products.map((prod) => (
                <tr key={prod.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-500">
                    {formatDistanceToNow(new Date(prod.createdAt), { addSuffix: true })}
                  </td>
