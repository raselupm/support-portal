...
export async function POST(
  ...
  try {
    ...
    if (chat.status === 'closed') return NextResponse.json({ error: 'Chat is already closed.' }, { status: 400 })
    ...
    const updatedChat: Chat = { ...chat, status: 'active', staffEmail, staffName, updatedAt: now }
    ...
    await pusherServer.trigger(chatChannel(id), EVT_STATUS_CHANGE, { status: 'active', staffEmail, staffName })
